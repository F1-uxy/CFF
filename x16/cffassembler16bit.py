        ...
        if (r >= 0 and r <= 7):
            if token[2].find('@') != -1:
                if token[0].upper().find('STRL') != -1:
                    addr = int(token[2][1:], 16)
                    o = [0x11, r, addr >> 8, addr & 0xFF]

                elif token[0].upper().find('STRH') != -1 :
                    addr = int(token[2][1: ], 16)
                    o = [0x12, r, addr >> 8, addr & 0xFF]
                    
                else:    
                    addr = int(token[2][1:], 16)
                    o = [0x10, r, addr >> 8, addr & 0xFF]
